function [xRes,iterNum] = single1(A,b,c)
M=1e6;
iterNum = 1;
[m,n] = size(A);
basisIndex = 1:m;
...
format rat;
writematrix([ basisOrder(basisIndex)' b ADisp; -1 -1 sigmaDisp],'matrix.txt','WriteMode','append')
% disp(iterNum);
[~,kk] = max(sigmaVector);

if all(sigmaVector <= 0)
   xRes(basisOrder) = x0; 
   break;
elseif all(A(:,kk)<=0)
   xRes = "unbounded";
   break;
    ...
    thetaVector = x0(basisIndex)./A(:,kk);
    thetaVector(thetaVector<=0) = M;
    [~,ll] = min(thetaVector);
    alk = A(ll,kk);
